...
class ComparisonSortingVisualizer {
    constructor(canvasID, numberOfElements, algorithm) {
        ...
        this.elements = [];
        this.speedRatio = 0.5;
        this.pivotIndicator = new Rect(-30, this.MARGIN_TOP - 12, 20, 10, 'pivot', null, null, 'black');
        this.lowIndex = new Rect(-100, this.MARGIN_TOP - 30, this.CELL_WIDTH, 10, 'h ', null, null, 'blue');
        this.highIndex = new Rect(-100, this.MARGIN_TOP - 30, this.CELL_WIDTH, 10, ' k', null, null, 'blue');
    ...
    */
    ...
    createNumbers(arr) {
        ...
        for (let i = 0; i < numOfElems; ++i) {
            ...
            let s = new Rect(this.MARGIN_LEFT + (this.CELL_WIDTH * i) + (this.CELL_SPACING * i), this.MARGIN_TOP, this.CELL_WIDTH, this.CELL_HEIGHT, arr[i], '#00796B', '5D4037', 'white');
    ...
    */
    ...
    quickSort_partition3(arr, startIndex, endIndex) {
        let pivot = arr[startIndex];
        let h = startIndex;
        let k = startIndex + 1;
        this.animationScript.push(["movePivot", startIndex]);
